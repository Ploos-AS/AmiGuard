...
a=argparse.Namespace(**{k:Path(v) for k,v in old['inputs'].items()},run_dir=Path('/tmp/amiguard-m22b-run'),cc='/opt/amiga/bin/m68k-amigaos-gcc',xdftool='/tmp/amiguard-m03-tools/bin/xdftool')
r=m.prepare(a)
...
record=json.loads((m.REPO/'signatures/files/amiguard-file-test-marker.json').read_text())
sig=record['signature']; pattern=bytes.fromhex(sig['bytes']); mask=bytes.fromhex(sig['mask']); offset=sig['offset']
assert record['status']=='test-only' and record['synthetic'] and set(sig)=={'offset','bytes','mask'}
positive=bytearray(offset+len(pattern)); positive[offset:]=pattern
miss=bytearray(positive); j=next(i for i,v in enumerate(mask) if v); bit=mask[j]&-mask[j]; miss[offset+j]^=bit
assert all((positive[offset+i]&v)==(pattern[i]&v) for i,v in enumerate(mask))
assert any((miss[offset+i]&v)!=(pattern[i]&v) for i,v in enumerate(mask))
for name,data in [('positive.bin',positive),('nearmiss.bin',miss)]:
 (r/'fixtures'/name).write_bytes(data); (r/'fixtures'/name).chmod(0o444)
meta=json.loads((r/'evidence/manifest.json').read_text()); meta['file_before']={p.name:m.digest(p) for p in (r/'fixtures').iterdir()}; meta['file_sizes']={p.name:p.stat().st_size for p in (r/'fixtures').iterdir()}
(r/'evidence/manifest.json').write_text(json.dumps(meta,indent=2)+'\n')
(r/'evidence/synthetic-fixture.json').write_text(json.dumps({'metadata':record,'mutated_offset':offset+j,'xor':bit,'positive_magic':positive[:4].hex(),'positive_hex':positive.hex(),'nearmiss_hex':miss.hex(),'size_constraint':'none beyond offset+length and 128 KiB intake cap'},indent=2)+'\n')
for name,target in [('positive','positive.bin'),('nearmiss','nearmiss.bin')]:
 (r/'work'/name).write_text(f'FailAt 21\nAmiGuard >AGTest:{name}.log FILE AGFiles:{target}\nProbe >AGTest:{name}-probe.log\nType AGTest:{name}.log\nEcho >AGTest:{name}.done "RETURNED"\n')
for name,target in [('positiveseries','positive.bin'),('nearmissseries','nearmiss.bin')]:
 lines=['FailAt 21',f'Probe >AGTest:{name}-before.log']
 for i in range(1,11):
  lines += [f'AmiGuard >AGTest:{name}-{i:02d}.log FILE AGFiles:{target}',f'Probe >AGTest:{name}-memory-{i:02d}.log',f'Echo "SCAN {i} RETURNED"']
 lines += [f'Echo >AGTest:{name}.done "RETURNED"']
 (r/'work'/name).write_text('\n'.join(lines)+'\n')
