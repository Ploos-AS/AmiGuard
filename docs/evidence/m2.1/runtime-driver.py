...
def execute(run):
    ...
    try:
        ...
        for name, expected, code in [('filevalid','VALID-HUNK:',0),('fileplain','NOT-HUNK:',0),('filemalformed','MALFORMED-HUNK:',0),('fileoversize','ERROR: file exceeds 128 KiB M2.1 limit',20),('filemissing','ERROR: cannot open file read-only',20)]:
            stage(name)
            require(name,expected,code)
        for name, expected in [('validseries','VALID-HUNK:'),('plainseries','NOT-HUNK:'),('malformedseries','MALFORMED-HUNK:')]:
            stage(name)
            for i in range(1,11):
...
if __name__ == '__main__':
    execute(Path('/tmp/amiguard-m21-final'))
