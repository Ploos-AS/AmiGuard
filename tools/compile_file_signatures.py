...
def validate(path, item):
    ...
    if item["status"] not in ("test-only", "research", "safe-test", "qualified", "verified"):
    ...
    elif status == "safe-test":
        if item["synthetic"]:
            fail(path, "safe-test records must describe non-synthetic harmless test artifacts")
        if not isinstance(sample_hash, str) or not SHA256_RE.match(sample_hash):
            fail(path, "safe-test records require sample_sha256")
        if item["cleaner"] != "none":
            fail(path, "safe-test records must not define a cleaner")
        validate_signature(path, sig)
    else:
        if item["synthetic"]:
...
def render(items):
    compiled = [item for item in items
                if item["status"] in ("test-only", "safe-test", "verified")]
    lines = [
    ...
    for item in compiled:
        ...
        test_only = 1 if item["status"] in ("test-only", "safe-test") else 0
